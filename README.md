...
[^1] We decided to take advantage of the existing architecture, ScanSSD.
This network is trained based on the SSD architecture. Although the
author published pre-trained weights, their training samples only
contain typesetting documents, which belong to a different population
that our model will face, meaning that simply using their weights will
not produce good results. In fact, we tested the weights trained by the
...
[^1]: Source code: https://github.com/mrr1vfe/cs539-math-symbol
